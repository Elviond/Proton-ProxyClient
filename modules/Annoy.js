...
var hack = new Module({
		...
		cmd: (sender, args, sendMessage, targetClient) => {
			if (args.length > 1) {
				annoy = args[1]
				sendMessage(sender, '&4[Proton]&8 Now annoying &2'+args[1]);
			} else {
				if (annoy != '') {
					annoy = ''
					sendMessage(sender, '&4[Proton]&8 Disabled annoy');
				} else {
				    sendMessage(sender, '&4[Proton]&8 You must supply who you with to annoy');
				}
			}
		},
		onDisable: () => {},
